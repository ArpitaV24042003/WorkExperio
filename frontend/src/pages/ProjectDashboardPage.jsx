...
import { apiRequest } from "../api";
import { aiChat } from "../ai"; // <- server-proxied AI chat

// --- Small modal used after finishing a task Q&A ---
const CompletionModal = ({ message, onClose }) => (
...
export default function ProjectDashboardPage() {
  ...
  // If you later pass a projectId in the route, you can fetch here:
  // const { id: projectId } = useParams();
  const projectId = null; // <- replace with real route param if you make this per-project

  // ------- Tasks & Q/A -------
  // In real usage, fetch tasks for projectId:
  ...
  const [selectedTask, setSelectedTask] = useState(null);
  const [qIndex, setQIndex] = useState(0); // 0 = instruction page, 1..N = questions
  const [answer, setAnswer] = useState("");
  ...
  const handleQuestionSubmit = async () => {
    setTaskError(null);
    if (answer.trim().length < 5) {
      setTaskError("Answer must be at least 5 characters.");
      return;
    }
    ...
    if (qIndex < selectedTask.questions.length) {
      ...
      // Persist answers to backend if you have an endpoint:
      // await apiRequest(`/projects/${projectId}/tasks/${selectedTask.id}/answers`, "POST", { answers: newList });
      ...
      // Optional: trigger AI review on server
      // await apiRequest(`/projects/${projectId}/tasks/${selectedTask.id}/review`, "POST", { answers: newList });
  ...
  // ------- Team Chat (local-only demo; hook to backend if you like) -------
  const [chatMessages, setChatMessages] = useState([
    {
      id: 1,
      user: "Supervisor",
      text: "Welcome to the chat! Coordinate your work here.",
      timestamp: "10:00",
    },
    {
      id: 2,
      user: "You",
      text: "Got it. I’ll start with the data model.",
      timestamp: "10:05",
    },
  ]);
  const [chatInput, setChatInput] = useState("");
  ...
  const handleSendChat = async () => {
    ...
    // await apiRequest(`/projects/${projectId}/chat`, "POST", msg);
  };

  // ------- AI Chatbot (server-proxied model) -------
  const [botMessages, setBotMessages] = useState([]);
  const [botInput, setBotInput] = useState("");
  const [isBotLoading, setIsBotLoading] = useState(false);
  ...
  const handleBotSubmit = async () => {
    ...
    setBotMessages((prev) => [...prev, { sender: "user", text: prompt }]);
    setBotInput("");
    setIsBotLoading(true);

    try {
      const systemPrompt =
        "You are the 'Project Assistant AI'. Provide concise, actionable guidance for academic projects.";
      const text = await aiChat({ prompt, systemPrompt });
      setBotMessages((prev) => [
        ...prev,
        { sender: "bot", text: text || "Sorry, something went wrong." },
      ]);
    } catch (e) {
      setBotMessages((prev) => [
        ...prev,
  ...
  // ------- Files (client-side mock; wire to backend FormData if you want) -------
  const [uploadedFiles, setUploadedFiles] = useState([]);
  const handleFileUpload = useCallback(async (e) => {
    ...
    // await apiRequest(`/projects/${projectId}/files`, "POST", form);
  }, []);

  const handleDeleteFile = useCallback(async (fileId) => {
    setUploadedFiles((prev) => prev.filter((f) => f.id !== fileId));
    // await apiRequest(`/projects/${projectId}/files/${fileId}`, "DELETE");
  }, []);
  ...
  // If you want to fetch per-project info and show here:
  // useEffect(() => {
  //   if (!projectId) return;
  //   (async () => {
  //     const project = await apiRequest(`/projects/${projectId}`);
  //     // Bind project.aiPlan.* into the "home" section if needed
  //   })();
  // }, [projectId]);
      ...
      case "home":
        return (
          <div className="p-8 bg-white rounded-xl shadow-lg">
            ...
            <p className="text-gray-600 text-lg mb-6">
              Use the navigation to manage tasks, chat with your team, and
            ...
            <div className="mt-8 p-6 bg-indigo-50 rounded-xl">
              ...
              <p className="text-gray-700">
                Bind this to your project’s AI plan (e.g.,{" "}
                <code>project.aiPlan.problemStatement</code>) if you route with
                an ID.
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 mt-6 border-t border-gray-200">
                  ...
                  <p className="text-gray-600 mt-2">
                    Once all tasks are completed, your AI review can be
        ...
        return (
          <div className="p-8 bg-white rounded-xl shadow-lg">
            ...
            {qIndex === 0 ? (
              <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-xl">
                <p className="text-lg text-gray-600 mb-6">
                  Finish the work for <strong>{selectedTask.title}</strong>.
                  When done, start the brief Q&A.
                </p>
                <button
                  onClick={handleFinishTask}
                  className="bg-green-600 text-white font-semibold px-8 py-3 rounded-xl shadow-lg hover:bg-green-700 transition"
                >
                  Task Finished — Start Q&A
                </button>
              </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      {showCompletionModal && (
        <CompletionModal
          message="Task answers submitted! (Hook server review as needed.)"
          onClose={() => setShowCompletionModal(false)}
        />
      )}
    </div>
  ...
  <div className="flex flex-col h-[85vh] bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <div className="flex-1 p-6 space-y-4 overflow-y-auto">
      {messages.length === 0 && (
        <div className="text-center p-8 text-gray-500">
          <p className="mb-2">
            Ask for advice, definitions, or project guidance.
          </p>
  ...
  return (
    <div className="p-8 bg-gray-50 rounded-xl shadow-lg">
      ...
      <div className="space-y-6">
        ...
        >
          Analysis of uploaded code structure, performance, and adherence to
          standards.
        </ReviewCard>
        ...
        >
          Evaluation of Q&A logs for completeness, comprehension, and relevance.
        </ReviewCard>
        ...
        >
          Peer feedback on implementation details and collaboration.
        </ReviewCard>
        ...
        >
          Quantitative scoring for effort, quality, and commitment.
        </ReviewCard>
      </div>
    </div>
  );
  ...
  <div className="p-8 bg-white rounded-xl shadow-lg">
    ...
    <div className="space-y-6">
      <div className="p-6 bg-indigo-50 rounded-xl border border-indigo-200">
        ...
        <p className="text-gray-700 mb-4">
          Combine AI scores and peer ratings to compute XP and levels on the
          server.
        </p>
        <div className="flex justify-around text-center mt-6">
          <div>
      ...
      <div className="p-6 bg-yellow-50 rounded-xl border border-yellow-200 text-center">
        ...
        <p className="text-lg font-semibold text-gray-700">
          Issue/download certificates once server confirms the level threshold.
        </p>
        <button className="mt-4 bg-yellow-500 text-white font-semibold px-6 py-2 rounded-lg hover:bg-yellow-600 transition">
          Download Certificate
        </button>
